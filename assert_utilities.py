...
def check_structure_dict_str_set_int(
    ...
    Args:
        student_answer (dict[str, set[int]]): The student's submitted answer, expected to be a dictionary where each key is a string and each value is a set of integers.
        instructor_answer (dict[str, set[int]]): The instructor's answer for reference.
    ...
    for k, v in student_answer.items():
        if not isinstance(v, set):
            msg_list.append(f"Element {k} of your dictionary should be of type 'set'")
            status = False
            continue

    # Check that all set elements are ints
    for v in student_answer.values():
        for e in v:
            if not isinstance(e, int | np.integer):
                msg_list.append(f"Element {e} of your set should be of type 'int'")
                msg_list.append("Check all other set elements")
                status = False
                break
...
def check_structure_dict_str_ndarray(
    ...
    if status:
        ...
        for s_key, s_value in student_answer.items():
            if s_key not in instructor_answer:
                continue
            vs = s_value
...
def check_structure_list_list_float(
    ...
    for i, s_list in enumerate(student_answer):
        ...
        for j, el in enumerate(s_list):
            if not isinstance(el, int | np.integer | float | np.floating):
                msg = (
...
def check_answer_scatterplot3d(student_answer, instructor_answer, options, validation_functions):
    ...
    rel_tol = options.get("rel_tol", 1.0e-2)
    ...
    status, msg = check_float(sum_s, sum_i, rel_tol=rel_tol, abs_tol=1.0e-5)
    msg_list.append(msg)
